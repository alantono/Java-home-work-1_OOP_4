...
public abstract class Archer extends Characters{
    ...
    public Archer(int attack, int defense, int damage_min, int damage_max, float hp, float hp_max, int speed, int x,
        ...
        super(attack, defense, damage_min, damage_max, hp, hp_max, speed, x, y, teams, coords.x, coords.y, state);
        this.shots = shots;
    }
...
public void step(ArrayList<Characters> teams_1, ArrayList<Characters> teams_2) {
    ...
    float damage = (teams_2.get(target).defense - attack > 0) ? damage_min : (teams_2.get(target).defense - attack < 0) ? damage_max : (damage_max + damage_min)/2;
    teams_2.get(target).getDamage(damage);
