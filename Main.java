...
// 3.Реализовать метод step() лучников.  3.1 Если жизнь равна нулю или стрел нет, завершить обработку. 
// 3.2 Поиск среди противников наиболее приближённого. 3.3 Нанести среднее повреждение найденному противнику. 
// 3.4 Найти среди своих крестьянина. 3.5 Если найден завершить метод иначе уменьшить запас стрел на одну.
...
// получаю список из 7 героев - teams
        ...
        teams.add(new Rogue(0, 0, 1, getName(), new Vector2D(0, 0), null));
...
// формирую первую команду teams-1 из 10ти героев (из крестьян, разбойников, снайперов и колдунов)
        ...
        if (((teams.get(rnd.nextInt(0,7))).getClass().getSimpleName()).equals("Rogue") && count <= 10 ) {
            teams_1.add(new Rogue(1, 0, 1, getName(), new Vector2D(0, 0), null)); count ++;}
...
// объединяю обе команды в один список
    ...
    for (Characters hero: allTeam) {
     ...
     // нажатие Enter
    public static void pressEnter(String[] args){
        Scanner sc = new Scanner(System.in);
        System.out.println("Для следующего шага нажмите Enter\nДля выхода нажмите любую клавишу + Enter");
        String entry = sc.nextLine();
        int t = 0;
        if(!"".equals(entry)) {
            System.out.printf("Сделано %d ходов", t);
            t ++;
        }
        else {

            System.out.printf("сделали %d ход", t);
            t ++;
        }}
